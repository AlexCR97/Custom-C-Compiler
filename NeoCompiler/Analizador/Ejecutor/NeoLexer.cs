...
namespace NeoCompiler.Analizador.Ejecutor
{
    public class NeoLexer
    {
        ...
        private List<NeoToken> tokens = new List<NeoToken>();
        public List<NeoToken> Tokens
        {
            get {
                return tokens;
            }
        ...
        public NeoLexer(string sourceCode)
        {
        ...
        {
            foreach (var i in NeoTokens.Tokens)
            {
                string lexeme = i.Key;
                string tokenRegex = i.Value;

                int end = NeoTokens.EndOfMatch(SourceCode.ToString(), tokenRegex);
                ...
                if (end != -1)
                {
                    ...
                    Tokens.Add(new NeoToken(CurrentLexeme, CurrentToken));
